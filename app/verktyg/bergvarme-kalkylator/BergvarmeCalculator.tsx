...
// Prismodell bergvärme 2026 (SEK, inkl. moms, innan rotavdrag)
// Bergvärme delas upp i Pump & Borrning/Installation
const PUMP_SIZES = {
...
// Borrning (kostar oftast 300-400 kr / meter)
const BOREHOLE_DEPTHS = {
  small: { min: 45000, max: 60000, name: "Kortare hål (ca 120-150 m)" },
  medium: { min: 60000, max: 80000, name: "Mellan djupt hål (ca 150-200 m)" },
  large: { min: 80000, max: 100000, name: "Djupt hål (över 200 m)" }
};
...
export default function BergvarmeCalculator() {
  ...
  const calculatePrice = useMemo(() => {
    ...
    const baseBorehole = BOREHOLE_DEPTHS[houseSize];
    ...
    // Arbetskostnad och maskin (Borrning + Installation)
    // Borrning och installation multipliceras med svårighet och region
    const laborMin = baseBorehole.min * diffMult * regMult;
    const laborMax = baseBorehole.max * diffMult * regMult;
    
    // ROT avdrag (35% schablon för bergvärme på totalbeloppet är vanligt, eller 30% på arbetskostnaden)
    // Vi använder Skatteverkets schablon för värmepumpar (35% av totalbeloppet anses vara arbetskostnad)
    // 30% ROT-avdrag på dessa 35% ger ca 10.5% rabatt på totalpriset
    ...
    <div className="max-w-4xl mx-auto py-8">
      <div className="grid md:grid-cols-2 gap-12">
        ...
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-slate-700 mb-2">Borrning & Tomtförutsättningar</label>
            <div className="flex bg-slate-100 p-1 rounded-xl">
              ...
              >
                Svårborrat (Djupt/Trångt)
              </button>
            </div>
            {difficulty === 'hard' && (
               <p className="text-xs text-amber-600 mt-2">Djupt till fast berg eller svåråtkomlig tomt driver upp arbetskostnaden.</p>
            )}
          </div>
        ...
        <div>
          {hasCalculated ? (
            <div className="bg-white border-2 border-[#0f766e] rounded-2xl p-6 shadow-xl shadow-[#0f766e]/10 animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <p className="text-sm text-slate-500 mb-6">Totalkostnad efter ROT-avdrag (inkl. borrning, pump & moms).</p>
            ...
            <div className="h-full bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center p-8 text-center text-slate-400">
              ...
              <p className="font-medium">Fyll i uppgifterna och klicka på beräkna för att se vad din bergvärmeanläggning borde kosta 2026.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="mt-20 max-w-3xl">
        <h2 className="text-2xl font-bold text-slate-900 mb-4">Att tänka på när du granskar offerter för bergvärme</h2>
        <div className="prose prose-slate">
          <p>
            Att installera bergvärme är en stor investering där arbetskostnaden (borrning och installation) ofta utgör 
            en lika stor del som själva värmepumpen. Det är viktigt att jämföra vad som faktiskt ingår i offerterna.
          </p>
          <ul className="space-y-2 mt-4 text-slate-700">
            <li><strong>Borrdjup och foderrör:</strong> Hur djupt ska de borra? Och hur många meter foderrör ingår? (Foderrör behövs ner till fast berg, och kostar ofta extra per meter om det krävs mer än vad som ingår).</li>
            <li><strong>ROT-avdrag schablon:</strong> För bergvärme tillämpar Skatteverket ofta en schablon där 35% av totalkostnaden anses vara arbetskostnad. Du får dra av 30% av den summan.</li>
            <li><strong>Demontering av gammal panna:</strong> Ingår det att de forslar bort din gamla värmepanna? Detta kan annars kosta flera tusen extra.</li>
            <li><strong>Återställning av tomt:</strong> Tunga maskiner på tomten kan ge skador. Ingår någon form av återställning?</li>
          </ul>
        </div>
      </div>
    </div>
